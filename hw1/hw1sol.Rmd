...
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F, '(NR>1)''{ print $6 }' | wc -l
```

**Solution:** Number of Unique patients.
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F, '(NR>1)''{ print $1 }' | sort | uniq | wc -l
```
...
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F, '(NR>1)''{ print $6 }' | sort | uniq
```

**Solution:** Possible values of 'admission_location'.
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F, '(NR>1)''{ print $7 }' | sort | uniq
```


**Solution:** Possible values of 'insurance'.
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F, '(NR>1)''{ print $9 }' | sort | uniq
```

**Solution:** Possible values of 'ethnicity'.
```{bash}
zcat /mnt/mimiciv/1.0/core/admissions.csv.gz | awk -F, '(NR>1)''{ print $12 }' | sort | uniq
```
...
**Solution:** The '>' overwrites an existing file with the newest output given. For example if 'test1.txt' already existed it would have been changed into 'hello, world'. However, the '>>' operator appends to an existing file so 'test2.txt' would add 'hello, world' to whatever was already in 'test2.txt'.
