...
```{r}
names(labevents_tble) <- c('subject_id', 'charttime', 'bicarbonate', 
...
```{r}
labevents_tble$subject_id <- as.character(labevents_tble$subject_id)
options(scipen = 999)

#lab_means <- colMeans(labevents_tble[sapply(labevents_tble, is.numeric)], na.rm=TRUE)

lab_means <- labevents_tble %>%
  summarise_at(c("bicarbonate", "chloride", "creatinine", "glucose", 
                 "potassium", "sodium", "hematocrit", "white_blood_cell_count",
                 "calcium", "magnesium"), 
                 mean, na.rm = TRUE)

lab_medians <- labevents_tble %>%
  summarise_at(c("bicarbonate", "chloride", "creatinine", "glucose", 
                 "potassium", "sodium", "hematocrit", "white_blood_cell_count",
                 "calcium", "magnesium"), 
                 median, na.rm = TRUE)

lab_sd <- labevents_tble %>%
  summarise_at(c("bicarbonate", "chloride", "creatinine", "glucose", 
                 "potassium", "sodium", "hematocrit", "white_blood_cell_count",
                 "calcium", "magnesium"), 
                 sd, na.rm = TRUE)

lab_min <- labevents_tble %>%
  summarise_at(c("bicarbonate", "chloride", "creatinine", "glucose", 
                 "potassium", "sodium", "hematocrit", "white_blood_cell_count",
                 "calcium", "magnesium"), 
                 min, na.rm = TRUE)

lab_max <- labevents_tble %>%
  summarise_at(c("bicarbonate", "chloride", "creatinine", "glucose", 
                 "potassium", "sodium", "hematocrit", "white_blood_cell_count",
                 "calcium", "magnesium"), 
                 max, na.rm = TRUE)
...
lab_measures <- rbind(lab_means, lab_medians, lab_sd, lab_min, lab_max)
lab_measures$statistic <- c("Mean", "Median", "sd", "min", "max")
lab_measures <- lab_measures %>% select(statistic, everything())
print.data.frame(lab_measures)
...
```{r}
names(chartevents_tble) <- c('subject_id', 'charttime', 'systolic_non-invasive_blood_pressure', 
                       'mean_non-invasive_blood_pressure', 'heart_rate', 'respiratory_rate', 'body_temperature_in_Fahrenheit')
```

```{r}
chart_means <- chartevents_tble %>%
  summarise_at(c("systolic_non-invasive_blood_pressure", "mean_non-invasive_blood_pressure", "heart_rate", "heart_rate", 
